...
def plot_hamming_weight_distribution(n):
  ...
  plt.scatter(x_values, actual_hamming_weights, marker='o', facecolors='none', edgecolors='#A0C4FF', s=20, label='Hamming distance $c(x)$')
  #plt.plot(x_values, actual_hamming_weights, 'ko', label='Actual hamming weight', markersize=1, alpha=0.7)
  plt.plot(x_values, lower_bounds, '-', label='Lower bound $c_l(x)$', linewidth=1, color='#1C6ECD')
  plt.plot(x_values, upper_bounds, '-', label='Upper bound $c_u(x)$', linewidth=1, color='#991C38')
  # plt.plot(x_values, expected_values, '--', label='Expected value $\\bar{c}(x)$', linewidth=1, color="#005955")
  
  plt.xlabel('$x$')
  plt.ylabel(f'Hamming distance')
  plt.title(f'Distribution of Hamming distance for $2^{{{n}}}-1$ (the number of zeros)')
  plt.legend()
