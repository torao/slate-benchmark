...
XY が保存されている CSV ファイルから散布図を描画します。Y が複数ある場合、それらのエラーバーを表示することが
できます。

使用方法:
    python scatter-plot-xy.py file1.csv file2.csv=Series2 file3.csv
    python scatter-plot-xy.py data.csv --title "実験結果" --xlabel "サンプル数" --yscale log
...
def plot_multiple_files(file_args, args):
    ...
    for i, file_arg in enumerate(file_args):
        ...
        if single or args.no_errorbars:
            # 平均値を折れ線グラフで表示
            plt.plot(
                stats["X"],
                stats["mean"],
                ...
                linewidth=1,
                markersize=4,
                label=f"{legend_name}{' (mean)' if not single else ''}",
            )
        else:
            # 平均値をエラーバー付きで表示
            sigma = "±σ" if args.no_latex else "$\\pm\\sigma$"
            plt.errorbar(
                stats["X"],
                stats["mean"],
                yerr=stats["error"],
                ...
                markersize=4,
                capsize=2,
                label=f"{legend_name} (mean{sigma})",
            )
    ...
    if not args.no_latex:
      plt.title(f"\\textbf{{{args.title}}}", fontsize=14, fontweight="bold")
    else:
...
使用例:
  ...
  %(prog)s results.csv --xmin 1 --xmax 1000 --ymin 0 --ymax 100
    ...
    parser.add_argument(
        "--title",
        default="Experimental Data Comparison",
        help="グラフタイトル (デフォルト: Experimental Data Comparison)",
    )
    parser.add_argument("--xlabel", default="X", help="X軸ラベル (デフォルト: X)")
    parser.add_argument("--ylabel", default="Y", help="Y軸ラベル (デフォルト: Y)")
    parser.add_argument(
        "--no-errorbars", action="store_true", help="エラーバー省略 (デフォルト: False)"
    )
    parser.add_argument(
        "--no-scatter", action="store_true", help="散布図省略 (デフォルト: False)"
    )
    parser.add_argument(
